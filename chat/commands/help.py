...
from chat.commands import register_command

# Try to import help_text, but don't fail if it's not there yet
try:
    from chat.commands.help_text import TOOL_COMMANDS_HELP
except ImportError:
    TOOL_COMMANDS_HELP = """
    ## Tool Commands
...
async def cmd_help(cmd_parts: List[str], context: Dict[str, Any]) -> bool:
    ...
    if len(cmd_parts) > 1 and cmd_parts[1].lower() in ("conversation", "ch"):
        from chat.commands.conversation_history import conversation_history_command
    ...
    if len(cmd_parts) > 1:
        ...
        from chat.commands import _COMMAND_HANDLERS
        ...
        if specific_cmd in _COMMAND_HANDLERS:
            ...
            from chat.commands import _COMMAND_COMPLETIONS
